...
| Método     | Mapping                     | Descripción                                                                 |
|------------|-----------------------------|-----------------------------------------------------------------------------|
| `getImage` | `@GetMapping("/{id}")`      | Recupera la imagen asociada a un anuncio específico identificado por su `id`. Devuelve la imagen en formato byte y establece los encabezados de respuesta para el tipo de contenido y la longitud de la imagen. Devuelve `404 Not Found` si el anuncio no existe. |
...
Una vez generado, redirige al `inicio de sesión`.
